...
class VehicleTypeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach($values as $value) {
            ...
            // Set service_id
            $value['service_id'] = $serviceIds[0] ?? null;

            // Remove non-existent/invalid columns from the main model creation
            unset($value['services']);
            unset($value['service_categories']);
            unset($value['min_per_unit_charge']);
            unset($value['max_per_unit_charge']);
            unset($value['cancellation_charge']);
            unset($value['waiting_time_charge']);
            unset($value['commission_type']);
            unset($value['commission_rate']);
